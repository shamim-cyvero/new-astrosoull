            ...
            <Stack w={'90%'} m={'40px auto'} alignItems={'center'} justifyContent={'center'} direction={{ base: 'column', md: 'column', lg: 'row' }}>
                <Stack w={'100%'} alignItems={'center'} justifyContent={'center'} direction={{ base: 'column', md: 'column', lg: 'row' }}>
                    ...
                    <VStack w={{ base: '100%', md: '100%', lg: '50%' }} spacing={3}>
                        <Heading children={'Astrologer SignUp'} fontSize={['1.2rem', '1.8rem']} />
                        ...
                        <Input type="password" fontSize={['.9rem','1rem']} fontWeight={'500'} required onChange={(e) => setConfirmPassword(e.target.value)}/>
                            ...
                            {/* <Checkbox defaultChecked mt={'10px'}  size={{base:'md',md:'sm'}} fontSize={['.9rem','1rem']}>I agree with <span style={{ fontWeight: "700", color: "#1f3b64" }}>terms & conditions</span></Checkbox> */}

                            <VStack w={'100%'}>
                                <Button colorScheme='whatsapp' type='submit' size={['sm', 'md']} w={'100%'} mt={'20px'} p={'2px 0px'} fontSize={'1.2rem'}>SignUp</Button>
                            </VStack>

                            <Text textAlign={'center'} mt={'15px'} fontWeight={'500'} fontSize={['.9rem', '1rem']}>Already have an Astrologer account? <NavLink to={'/astrologerlogin'} color={'green'} fontWeight={'900'} fontSize={['.9rem', '1rem']}>Login</NavLink></Text>
                            <HStack m={'1vmax 0'} w={'100%'} justifyContent={'center'} alignItems={'center'}>
                                <Button onClick={() => navigate('/signup')} alignSelf={'center'} children={'User SignUp'} size={['sm', 'md']} p={'2px 0px'} fontSize={'1.2rem'} _hover={{ bg: "#1f3b64", cursor: "pointer", color: 'white' }} />
                            </HStack>
                        </form>
                    </VStack>
                </Stack>
